...
export function CoinDetail({ row, interval, onClose }: { row: CoinRow; interval: IntervalId; onClose: () => void }) {
  ...
  const ai = useQuery({
    queryKey: ["chart-ai", row.base, chartInterval, lang],
    queryFn: () => analyzeChartAi({ data: { base: row.base, interval: chartInterval, lang } }),
    enabled: aiOpen,
    staleTime: 180_000,
    retry: 0,
  });
  ...
  const simple = useQuery({
    queryKey: ["chart-ai-simple", row.base, chartInterval, lang, levels?.verdict ?? null],
    queryFn: () =>
      explainSimple({
        data: {
          base: row.base,
  ...
  const news = useQuery({
    queryKey: ["coin-news", row.base, lang],
    queryFn: () => getNews({ data: { base: row.base, lang } }),
    staleTime: 120_000,
    retry: 0,
  });
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/65 p-3 backdrop-blur-sm sm:items-center sm:p-4"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
      aria-label={t("detail.dialog", { base: row.base })}
    >
      <div className="fade-up w-full max-w-4xl rounded-2xl bg-bg/95 shadow-[var(--shadow-border)]" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="max-h-[82vh] overflow-y-auto p-4">
          ...
          {live && !chartFailed ? (
            <div className="mt-3 grid gap-3 md:grid-cols-[1fr_1.3fr]">
          ...
          {aiOpen && levels && !ai.isFetching ? (
            <div className="fade-up mt-3 overflow-hidden rounded-xl bg-surface">
              ...
              <div className="flex flex-col gap-4 p-4">
                ...
                <div className="border-t border-border pt-3">
                  ...
                  {simpleOpen && simple.data?.ok && !simple.isFetching ? (
                    <p className="mt-2 text-sm leading-relaxed text-fg">{stripMd(simple.data.text)}</p>
                  ) : null}
          ...
          <div className="mt-5">
            <p className="text-xs font-medium tracking-wide text-faint uppercase">{t("detail.newsAbout", { base: row.base })}</p>
            <ul className="mt-2 flex flex-col gap-2">
              {news.isLoading ? (
                <li className="h-12 animate-pulse rounded-lg bg-surface-2" />
              ) : news.data?.length ? (
                news.data.slice(0, 6).map((item) => (
                  <li key={item.url || item.title}>
                    <a
                      ...
                      className="block rounded-lg bg-surface-2 p-2.5 text-xs text-fg hover:bg-surface-2/70"
                    >
                      {item.tone ? (
                        <span
