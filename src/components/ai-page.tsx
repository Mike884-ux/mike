      ...
      <div className="flex gap-1 rounded-xl bg-surface-2 p-1">
        {options.map((o) => (
          <button
            ...
            onClick={() => onChange(o)}
            className={cn(
              "h-9 flex-1 rounded-lg px-2 text-sm font-semibold",
              value === o
...
function PlanView({ plan }: { plan: StrategyPlan }) {
  ...
  return (
    <div className="fade-up mt-4 flex flex-col gap-5 rounded-2xl bg-surface p-5 shadow-[var(--shadow-border)]">
          ...
          <ul className="mt-3 flex flex-col gap-2">
            {plan.allocation.map((a, i) => (
              <li key={a.asset} className="flex gap-2.5 text-sm">
                ...
                />
                <span className="w-12 shrink-0 font-semibold text-fg tabular-nums">
                  {a.sharePct}%
                </span>
                <span className="text-fg">
                  <span className="font-semibold">{a.asset}</span>{" "}
                  <span className="text-muted">— {stripMd(a.why)}</span>
                </span>
              </li>
            ))}
          </ul>
      ...
      <ListBlock
        title={t("strategy.portfolio")}
        items={plan.portfolioNotes}
        marker="▸"
        tone="text-accent"
      />
...
const RISK_NAMES: Record<Risk, MessageKey> = {
  low: "strategy.risk.low",
  medium: "strategy.risk.medium",
  high: "strategy.risk.high",
};
const HORIZON_NAMES: Record<Horizon, MessageKey> = {
  short: "strategy.horizon.short",
  medium: "strategy.horizon.medium",
  long: "strategy.horizon.long",
};
    ...
    <div className="mx-auto w-full max-w-3xl">
      ...
      <div className="mt-4 grid gap-3 rounded-2xl bg-surface p-4 shadow-[var(--shadow-border)] sm:grid-cols-2">
        ...
        {locked ? (
          <Link
            to="/pricing"
            className="bg-brand flex h-11 items-center justify-center gap-2 rounded-xl text-sm font-semibold text-white shadow-[var(--shadow-glow)] sm:col-span-2"
          >
      ...
      {result && !result.ok && !advice.isPending ? (
        <AiFailure reason={result.reason} className="mt-4" />
      ) : null}
      ...
      {!result && !advice.isPending ? (
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          {(["strategy.tip1", "strategy.tip2", "strategy.tip3"] as MessageKey[]).map((key) => (
            <p
              key={key}
              className="rounded-2xl bg-surface p-4 text-sm leading-relaxed text-muted shadow-[var(--shadow-border)]"
            >
              {t(key)}
            </p>
          ))}
...
export function AiPage({ question, tab }: { question?: string; tab: "chat" | "strategy" }) {
  ...
  return (
    <div>
      <div
        className="mx-auto mb-4 flex w-full max-w-3xl gap-1 rounded-xl bg-surface-2 p-1"
        role="tablist"
      >
        {tabs.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                "flex h-10 flex-1 items-center justify-center gap-2 rounded-lg text-sm font-semibold",
                active
                  ? "bg-surface text-fg shadow-[var(--shadow-border)]"
                  : "text-muted hover:text-fg",
              )}
            >
              ...
              {t(item.label)}
              {item.id === "strategy" ? (
                <span className="rounded-md bg-primary/12 px-1.5 py-0.5 text-[10px] font-bold text-primary">
                  PRO
                </span>
              ) : null}
            </Link>
