...
// DATABASE_URL, POSTGRES_URL or a custom-prefixed name from Vercel's storage integrations.
const rawDatabaseUrl = typeof process !== "undefined" ? findDatabaseUrl(process.env) : undefined;
...
async function createPgliteSql(): Promise<Sql> {
  globalRef.__pgliteInstance__ ??= (async () => {
    ...
    try {
      ...
      /* stale pid from a killed process */
    }
    const pg = new PGlite({
    ...
    const onStop = () => {
      void pg.exec("CHECKPOINT").then(() => pg.close()).catch(() => undefined);
    };
    process.once("beforeExit", onStop);
    process.once("SIGTERM", onStop);
    const tick = setInterval(() => {
      void pg.exec("CHECKPOINT").catch(() => undefined);
    }, 20_000);
    tick.unref?.();
    return pg;
  })().catch((err) => {
